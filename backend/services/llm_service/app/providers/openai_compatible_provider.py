...
class OpenAICompatibleProvider(LLMProvider):
    ...
    async def rerank(
        ...
        instructions = (
            ...
            "Твоя задача — определить, какие тест-кейсы действительно проверяют данные изменения. "

            "ВАЖНО:\n"
            "1. Если тест-кейс НЕ связан с изменениями — явно укажи is_relevant = false.\n"
            "2. НЕ пытайся искусственно связать тест-кейс с изменениями.\n"
            "3. Если изменения не покрываются ни одним тест-кейсом — верни пустой список items.\n"
            "4. В explanation обязательно укажи причину:\n"
            "   - либо почему тест релевантен\n"
            "   - либо почему он НЕ относится к изменениям\n"
            ...
            "- explanation на русском языке (конкретное и краткое)\n"

            "Если связь слабая или косвенная — это НЕ релевантный тест."
        )
